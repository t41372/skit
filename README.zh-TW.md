...
提示詞條目是給 AI coding agent 的、可重複使用且帶參數的一段文字。加入一個 `.prompt.md` 檔（或用 `skit add --prompt` 直接起草），每個 `{佔位符}` 都會成為表單欄位，完整享有組合 / 上次值 / `--set` 體驗；`{{` 和 `}}` 寫出字面大括號，未納管的佔位符原樣保留。執行時 skit 渲染文字並開啟 agent 自己的互動 session——**執行器**在執行表單上選（或按提示詞釘選），claude / codex / opencode / amp / antigravity 已預先設定，其他工具用 `skit runner add mycli -- mycli run {prompt}` 註冊。渲染後的提示詞以單一程序參數傳遞——中間沒有 shell——多行文字和奇怪字元都逐位元組原樣到達。一個誠實的提醒：提示詞不是保密通道——渲染後的內容會落在對方 agent 自己的 session 紀錄裡。
