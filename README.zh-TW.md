...
- **收納腳本與提示詞**。`skit add` 把散落各處的腳本與提示詞收進同一個可搜尋的工具庫——複製一份收進庫裡，或用引用模式直接指向原檔。
...
- **提示詞是工具庫中的一等條目**。存一份帶參數的提示詞（納管的 `{{佔位符}}` 會變成表單欄位），一鍵丟給你的 coding agent——claude、codex、opencode 等執行器已預先設定好，其他 CLI 一句 `skit runner add` 就能接上。
