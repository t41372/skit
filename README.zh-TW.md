...
- **收納腳本與提示詞**。`skit add` 把散落各處的腳本與提示詞收進同一個可搜尋的工具庫——複製一份收進庫裡，或用引用模式直接指向原檔。不確定要加什麼？直接跑 `skit add`，它會問你；丟一個它認不出種類的檔案給它，它也是問你，而不是丟一句錯誤了事。
- **參數不再折磨人**。旗標、`input()`、你勾選管理的常數，全部變成表單欄位（choices 變選擇器、布林變勾選框、路徑會邊打邊補全還能開檔案瀏覽器、型別自動把關）。
- **路徑不用手打**。每個文字欄位旁邊都有一個 `📁 瀏覽`，點下去就是檔案選擇器；路徑欄位還會打到一半就提示補全，按 `→` 直接採用。從此不用手打目錄。（鍵盤走 `Ctrl+T` →〈檔案或資料夾…〉，開的是同一個選擇器。）
- **它會記住**。腳本預設值直接填進表單，腳本改了也跟著變；上次填的值自動帶回，`↺ 預設值`（Ctrl+O）一鍵改回腳本自己的值。常用的一組存成具名組合。標記為機密的參數永不保存——記住的值、組合、執行歷史裡都不會有它。`{cwd}`、`{today}` 這類值 token 讓同一個組合跨機器、跨目錄通用。
...
- **提示詞是工具庫中的一等條目**。存一份帶參數的提示詞（交給 skit 管理的 `{{佔位符}}` 會變成表單欄位），一鍵丟給你的 coding agent——claude、codex、opencode 等執行器已預先設定好，其他 CLI 一句 `skit runner add` 就能接上。
- **也是你的 agent 的工具庫**。官方 [Agent Skill](https://agentskills.io) 教會 Claude Code、Codex、Cursor、Gemini CLI 等 agent 完整用法：用 `skit list` 探索條目、用 `skit show` 讀參數結構、用 `skit run --set … --no-input` 執行。一句 `skit agent install` 就裝好——見[給你的 AI agent 用](#給你的-ai-agent-用)。
...
| ![工具庫](https://raw.githubusercontent.com/t41372/skit/main/docs/assets/tui-library-zh.png) | ![執行表單](https://raw.githubusercontent.com/t41372/skit/main/docs/assets/tui-form-zh.png) |
|:--:|:--:|
| **工具庫**——每個動作都在畫面上，滑鼠鍵盤皆可 | **執行表單**——從腳本自己的參數生成 |
...
| **加入腳本**——靜態偵測參數，勾選即管理 | **腳本設定**——參數、機密、組合、依賴 |
...
| **提示詞**（prompt） | 你的 agent CLI（claude · codex · …） | `{{name}}` 佔位符 | — | — | needs |

每種類型也都能手動**宣告**參數，所以連程式和命令範本都享有同樣的表單 / 組合 / `--set` 體驗。**needs** 是 skit 在執行前檢查是否位於 `PATH` 上的外部命令（任何類型皆適用）。Python 和 JS/TS 都有按腳本隔離的依賴套件：uv 解析 PEP 723 區塊，npm 式依賴則安裝到存副本旁的 `node_modules`（`skit add` 會從腳本自己的 import 建議清單）。JS/TS 依賴管理僅適用於複製進庫的條目——reference 模式的腳本沿用自己專案的 `node_modules`——且安裝一律不執行套件的 lifecycle scripts（npm 和 bun 加 `--ignore-scripts`；deno 預設就不跑）。另一個拉平差異的點：skit 自動選中 deno 時會帶 `--allow-all`，同一支腳本在 deno、bun、node 下行為一致。skit 會替 Python 引導 uv，但不會替你安裝 JS runtime——node、bun 或 deno 得你自己備好。

### 提示詞

提示詞條目是給 AI coding agent 的、可重複使用且帶參數的一段文字。加入一個 `.prompt.md` 檔（或用 `skit add --prompt` 直接起草）後，互動加入時可在複核頁中選擇哪些偵測到的 `{{佔位符}}` 要變成表單欄位。候選不超過 30 個時預設全選；超過 30 個時預設一個都不選，以免把程式碼範例誤認為變數。管理中的欄位完整享有組合 / 上次值 / `--set` 體驗。沒有任何轉義規則要學：凡是你沒交給 skit 管理的內容——包括沒管理的 `{{佔位符}}`——一律逐位元組原樣送達 agent；每個提示詞還有一個插值總開關（`--no-interpolate`，或條目設定裡的一個勾選框），寫作時沒考慮插值的長提示詞可以整個關掉。執行時 skit 會渲染文字，並把它交給所選執行器設定的 argv 命令。預設的 Claude、Codex、OpenCode 和 Antigravity 命令會開啟各自的互動 session；Amp 是刻意保留的例外，依其官方 `amp -x` 單次模式執行提示詞後返回。**執行器**在執行表單上選（或按提示詞釘選），claude / codex / opencode / amp / antigravity 已預先設定，其他工具用 `skit runner add mycli -- mycli run {{prompt}}` 註冊。渲染後的提示詞以單一程序參數傳遞——中間沒有 shell。一個誠實的提醒：提示詞不是保密通道——渲染後的內容會落在對方 agent 自己的 session 紀錄裡。

```bash
skit add review.prompt.md            # 管理中的佔位符變成表單欄位
skit run review                      # 選 agent、填表單、出發
