...
- **提示詞也是腳本**。存一份帶參數的提示詞(`{{佔位符}}` 會變成表單欄位),一鍵丟給你的 coding agent——claude、codex、opencode 等執行器已預先設定好,其他 CLI 一句 `skit runner add` 就能接上。
...
提示詞條目是給 AI coding agent 的、可重複使用且帶參數的一段文字。加入一個 `.prompt.md` 檔（或用 `skit add --prompt` 直接起草），每個 `{{佔位符}}`（雙大括號，所以塞滿單大括號的程式碼片段不會誤觸發）都會成為表單欄位，完整享有組合 / 上次值 / `--set` 體驗。沒有任何轉義規則要學：凡是你沒納管的內容——包括沒納管的 `{{佔位符}}`——一律逐位元組原樣送達 agent；每個提示詞還有一個插值總開關（`--no-interpolate`，或腳本設定裡的一個勾選框），寫作時沒考慮插值的長提示詞可以整個關掉。執行時 skit 渲染文字並開啟 agent 自己的互動 session——**執行器**在執行表單上選（或按提示詞釘選），claude / codex / opencode / amp / antigravity 已預先設定，其他工具用 `skit runner add mycli -- mycli run {{prompt}}` 註冊。渲染後的提示詞以單一程序參數傳遞——中間沒有 shell。一個誠實的提醒：提示詞不是保密通道——渲染後的內容會落在對方 agent 自己的 session 紀錄裡。

```bash
skit add review.prompt.md            # 佔位符變成表單欄位
