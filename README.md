...
A prompt entry is a reusable, parameterized piece of text for an AI coding agent. Add a `.prompt.md` file (or draft one with `skit add --prompt`), and every `{placeholder}` becomes a form field with the full preset / last-values / `--set` experience; `{{` and `}}` write literal braces, and unmanaged holes pass through untouched. Running one renders the text and opens your agent's own interactive session with it — the **runner** is picked on the run form (or pinned per prompt), claude / codex / opencode / amp / antigravity come preconfigured, and `skit runner add mycli -- mycli run {prompt}` registers anything else. The rendered prompt travels as a single process argument — no shell in between — so multi-line prompts and strange characters arrive byte-for-byte intact. One honest caveat: prompts are not a secrets channel — whatever the rendered text contains ends up in the receiving agent's own session logs.
