...
const CLOSED: &[(&str, &str)] = &[
    ...
    ("test_importtime", "Python-only -X importtime tree parser; the native Rust imports suite emits an explicit not-applicable import-time artifact because no Python import tree exists"),
    ("test_pyperf", "Python-only pyperf JSON parser; the Rust microbenchmark lane uses the native Criterion/CodSpeed harness and never consumes a pyperf artifact"),
    ("test_pyperf_parser_rejects_malformed_elements", "Python-only pyperf JSON parser validation; the Rust microbenchmark lane uses the native Criterion/CodSpeed harness and never parses Python pyperf benchmark elements"),
    ("test_pyperf_inherit_covers_the_fixture_vars", "Python pyperf worker inheritance contract; the native Rust benchmark harness has no pyperf worker process and constructs the complete child environment directly"),
    ("test_ci_runner", "Python pure helper accepted an injected environment mapping; Rust collect_meta reads BENCH_CI_RUNNER/ImageVersion through a private non-empty process-environment helper with no map-injection seam"),
    ("test_cpu_model", "Python parsed injected /proc/cpuinfo text; Rust collect_meta gets the CPU brand from sysinfo and exposes no text-parser seam"),
    ("test_mem_and_git", "Python exposed pure memory-page and git-status text helpers; Rust collect_meta uses sysinfo total memory plus a bounded live git status process and exposes neither parser seam"),
    ("test_dist_version_fallback", "Python queried importlib.metadata distribution versions; the native Rust benchmark harness has no Python distribution-metadata lookup and records native harness provenance instead"),
    ("test_build_host_and_meta", "Python exposed pure injected HostInfo/Meta builder helpers; Rust collect_meta intentionally constructs them from bounded live probes and sysinfo, while typed Meta round-trip remains executable"),
    ("test_deterministic", "Python benchmark source generation exposed a per-call seed injection; the Rust-only benchmark generator owns a fixed stable seed derived from language and line count and exposes no alternate-seed seam"),
...
const OWNERS: &[&str] = &[
    ...
    "crates/skit-benchmarks/tests/port_test_benchmarks_tooling_footprint_retry.rs",
