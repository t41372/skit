...
#[test]
fn test_write_prompt_managed_and_runner_roundtrip(){let s=Sandbox::new();let req=s.review("p","{{a}} {{b}}\n").create_entry().unwrap();FileStore::new(s.data.path()).create(req).unwrap();let rm=s.run(&["params","p","--rm","a"]);assert_eq!(rm.status.code(),Some(0),"{}",combined(&rm));let pin=s.run(&["params","p","--runner","claude"]);assert_eq!(pin.status.code(),Some(0),"{}",combined(&pin));let view=s.params("p");assert_eq!(view["placeholders"],serde_json::json!(["b"]));assert_eq!(view["runner"],"claude");let rm=s.run(&["params","p","--rm","b"]);assert_eq!(rm.status.code(),Some(0),"{}",combined(&rm));let clear=s.run(&["params","p","--runner",""]);assert_eq!(clear.status.code(),Some(0),"{}",combined(&clear));let view=s.params("p");assert_eq!(view["placeholders"],serde_json::json!([]));assert!(view["runner"].is_null());}
