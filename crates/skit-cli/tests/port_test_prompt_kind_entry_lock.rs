...
#[test]
fn test_prompt_store_writers_share_entry_lock_across_processes(){let s=Sandbox::new();s.add_prompt();let lock=s.meta_lock();let mut child=s.spawn(&["params","p","--runner","claude"]);assert_blocked(&mut child);lock.unlock().unwrap();wait_ok(child);assert_eq!(s.show()["runner"],"claude");}

#[test]
fn test_concurrent_prompt_runner_update_preserves_disjoint_metadata(){let s=Sandbox::new();s.add_prompt();let lock=s.meta_lock();let mut runner=s.spawn(&["params","p","--runner","claude"]);let mut desc=s.spawn(&["describe","p","updated description"]);assert_blocked(&mut runner);assert_blocked(&mut desc);lock.unlock().unwrap();wait_ok(runner);wait_ok(desc);let view=s.show();assert_eq!(view["runner"],"claude");assert_eq!(view["description"],"updated description");}

#[test]
fn test_concurrent_prompt_managed_update_preserves_disjoint_metadata(){let s=Sandbox::new();s.add_prompt();let lock=s.meta_lock();let mut managed=s.spawn(&["params","p","--rm","a"]);let mut desc=s.spawn(&["describe","p","managed changed"]);assert_blocked(&mut managed);assert_blocked(&mut desc);lock.unlock().unwrap();wait_ok(managed);wait_ok(desc);assert_eq!(fields(&s.show()),["b"]);assert_eq!(s.show()["description"],"managed changed");}
