...
impl Sandbox {
    ...
    fn seed_public_last(&self, slug: &str, name: &str, value: &str) {
        let slug = Slug::parse(slug).unwrap();
        let mut declaration = ParamDecl::new(name);
        declaration.delivery = ParameterDelivery::Env;
        let service = FormStateService::new(FileFormStateStore::new(self.state.path()));
        service
            .save_last(
                &slug,
                &[declaration],
                Some(&BTreeMap::from([(name.to_owned(), value.to_owned())])),
                None,
                false,
            )
            .unwrap();
...
fn test_cli_add_flag_param_on_exe_then_run_set() {
    ...
    let output = sandbox.ok(&[
        "params", "prog", "--add", "width", "--type", "width=int", "--deliver", "width=flag", "--flag", "width=--width", "--default", "width=800",
    ]);
    assert!(combined(&output).contains("Declared parameters: width"), "{}", combined(&output));
    ...
    let dry = sandbox.ok(&[
        "run", "prog", "--set", "width=1024", "--dry-run", "--no-input",
    ]);
...
fn test_cli_exe_show_table_and_json() {
    ...
    sandbox.add_exe("prog");
    sandbox.ok(&[
        "params", "prog", "--add", "width", "--deliver", "width=flag", "--flag", "width=--width", "--type", "width=int", "--default", "width=800",
    ]);
...
fn test_cli_exe_show_without_declared_is_plain_message() {
    ...
    assert!(
        String::from_utf8_lossy(&output.stdout).contains("has no managed parameters"),
        "{}",
        combined(&output)
    );
}
...
fn test_cli_python_manage_with_json_emits_the_final_read_view() {
    ...
    let document = sandbox.json(&[
        "params", "job", "--manage", "CITY", "--json",
    ]);
    assert_eq!(
        document["params"]
            .as_array()
...
fn test_cli_add_choice_placeholder_on_command_then_run() {
    ...
    sandbox.ok(&[
        "params", "conv", "--add", "size", "--type", "size=choice", "--choices", "size=s,m,l", "--default", "size=m", "--optional", "size",
    ]);
    ...
    assert!(
        String::from_utf8_lossy(&run.stdout).contains("m"),
...
fn test_cli_command_show_enriched_and_env_rider() {
    ...
    sandbox.ok(&[
        "params", "c", "--add", "msg", "--type", "msg=str", "--default", "msg=hi", "--optional", "msg",
    ]);
    sandbox.ok(&[
        "params", "c", "--add", "RETRIES", "--deliver", "RETRIES=env",
    ]);
    let human = sandbox.ok(&["params", "c"]);
    let text = String::from_utf8_lossy(&human.stdout);
    assert!(text.contains("msg") && text.contains("optional") && text.contains("RETRIES"), "{text}");
...
fn test_cli_command_env_rider_only_no_placeholders() {
    ...
    sandbox.ok(&[
        "params", "noph", "--add", "RETRIES", "--deliver", "RETRIES=env",
    ]);
    let output = sandbox.ok(&["params", "noph"]);
    assert!(String::from_utf8_lossy(&output.stdout).contains("RETRIES"), "{}", combined(&output));
}
...
fn test_cli_python_declared_op_is_refused() {
    ...
    assert!(combined(&output).contains("manages its parameters from the script itself"), "{}", combined(&output));
}
...
fn test_cli_secret_declared_env_purges_prior_plaintext() {
    ...
    sandbox.ok(&[
        "params", "prog", "--add", "TOKEN", "--deliver", "TOKEN=env",
    ]);
    sandbox.seed_public_last("prog", "TOKEN", "plaintext");
    assert_eq!(sandbox.json(&["params", "prog", "--json"])["last_values"]["TOKEN"], "plaintext");

    let output = sandbox.ok(&["params", "prog", "--secret", "TOKEN"]);
    assert!(combined(&output).contains("Removed previously stored plaintext"), "{}", combined(&output));
    let state = sandbox.json(&["params", "prog", "--json"]);
    assert!(state["last_values"].get("TOKEN").is_none(), "{state}");
}
...
fn test_cli_declared_secret_env_source_resolves_without_prompting() {
    ...
    sandbox.ok(&[
        "params", "svc", "--add", "TOKEN", "--deliver", "TOKEN=env", "--secret", "TOKEN", "--env-source", "TOKEN=SVC_TOKEN",
    ]);
    ...
    assert!(state["last_values"].get("TOKEN").is_none(), "secret leaked into state: {state}");
}
...
fn test_cli_run_set_env_and_placeholder_dry_run() {
    ...
    let output = sandbox.ok(&[
        "run", "dr", "--set", "msg=hello", "--dry-run", "--no-input",
    ]);
    assert!(String::from_utf8_lossy(&output.stdout).contains("RETRIES=3"), "{}", combined(&output));
}
...
fn test_cli_exe_show_masks_secret_default_and_last_value() {
    ...
    sandbox.add_exe("prog");
    sandbox.ok(&[
        "params", "prog", "--add", "a", "--secret", "a", "--add", "b", "--secret", "b", "--default", "b=x",
    ]);
    let path = sandbox.state_path("prog");
    ...
    assert!(!text.contains("stale") && !text.contains(" x "), "secret material leaked:\n{text}");
}
...
fn test_cli_command_show_masks_secret_placeholder_and_undeclared() {
    ...
    sandbox.ok(&[
        "params", "lg", "--secret", "password", "--default", "password=seed", "--required", "password",
    ]);
    let path = sandbox.state_path("lg");
    fs::create_dir_all(path.parent().unwrap()).unwrap();
    fs::write(&path, "[values]\npassword = \"stale\"\n").unwrap();
    ...
    let text = String::from_utf8_lossy(&output.stdout);
    assert!(text.contains("•••"), "{text}");
    assert!(!text.contains("seed") && !text.contains("stale"), "secret material leaked:\n{text}");
...
fn test_declared_add_on_interpreted_kind_delivers_at_run() {
    ...
    sandbox.ok(&[
        "params", "rb2", "--add", "SIZE", "--flag", "SIZE=--size",
    ]);
    let output = sandbox.ok(&[
        "run", "rb2", "--set", "SIZE=5", "--dry-run", "--no-input",
    ]);
...
fn test_declared_param_on_an_interpreted_kind_actually_delivers() {
    // Python uses a run_entry spy here, so its observable is the launch argv before the child
    // executes. Rust's public dry-run is the same post-form/post-delivery planner surface.
    let sandbox = Sandbox::new();
    sandbox.add_ruby("rb4");
    sandbox.ok(&[
        "params", "rb4", "--add", "GREETING", "--flag", "GREETING=--greeting",
    ]);
    let output = sandbox.ok(&[
        "run", "rb4", "--set", "GREETING=world", "--dry-run", "--no-input",
    ]);
    let text = String::from_utf8_lossy(&output.stdout).replace('\n', "");
    assert!(text.contains("--greeting") && text.contains("world"), "{text}");
}
