...
fn responsive_library_accepts_real_sgr_mouse_and_uses_selective_redraws() {
    ...
    assert!(
        ...
        "the terminal did not enable focus reporting"
    );
...
fn bare_add_uses_the_typed_workflow_and_returns_the_created_entry() {
    ...
    tui.wait_for("Source path");
    tui.send(path_input.as_bytes());
    let review = tui.checkpoint();
    tui.send(b"\r");
    tui.wait_for_after(review, "[Ctrl+S] Add");
    tui.send(b"\x13");
    let (code, output) = tui.wait_for_exit_status_after(0);
...
fn terminal_authoring_and_confirmation_paths_need_no_hidden_cli_knowledge() {
    ...
    tui.wait_for("Source path");
    tui.send(source_input.as_slice());
    let review = tui.checkpoint();
    tui.send(b"\r");
    tui.wait_for_after(review, "[Ctrl+S] Add");
    tui.send(b"\x13");
