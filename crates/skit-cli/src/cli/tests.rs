...
fn params_host_updates_managed_secrets_and_skips_an_invalid_environment_source_without_writes() {
    const CHILD: &str = "SKIT_PARAMS_HOST_TEST_CHILD";
    if std::env::var_os(CHILD).is_none() {
        ...
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "cli::tests::params_host_updates_managed_secrets_and_skips_an_invalid_environment_source_without_writes",
                "--nocapture",
            ])
            .env(CHILD, "1")
        ...
        assert!(state.join(".locks/managed-shell.values.lock").is_file());
        return;
    }
