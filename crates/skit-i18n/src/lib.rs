...
const CATALOG: &[Translation] = &[
    ...
    row!(
        ...
        "把脚本、可执行文件、提示词或命令加入 skit",
        "把腳本、可執行檔、提示詞或命令加入 skit",
    ),
    row!(
    ...
    row!(
        ...
        "在编辑器中打开脚本或提示词来源（名称不存在时可创建脚本）",
        "在編輯器中開啟腳本或提示詞來源（名稱不存在時可建立腳本）",
    ),
    row!(
    ...
    row!(
        ...
        "移除“runner list --all”中的一个原始行索引（或“container”）",
        "移除「runner list --all」中的一個原始列索引（或「container」）",
    ),
    ...
    row!(
        ...
        "读取或设置 skit 的设置（语言、编辑器、镜像、表单形态、运行后行为）",
        "讀取或設定 skit 的設定（語言、編輯器、鏡像、表單形態、執行後行為）",
    ),
    row!(
        "Manage the agents (runners) that prompt entries run with",
        "管理提示词条目使用的执行器(AI agent)",
        "管理提示詞項目使用的執行器(AI agent)",
    ),
    row!(
    ...
    row!(
        ...
        "把 skit 接上 AI agent:安装官方 Agent Skill",
        "把 skit 接上 AI agent:安裝官方 Agent Skill",
    ),
    row!(
    ...
    row!(
        "State commit failed: {}. Rollback also failed: {}.",
        "状态提交失败：{}。回滚也失败：{}。",
        "狀態提交失敗：{}。復原也失敗：{}。",
    ),
    row!(
    ...
    row!(
        ...
        "为 exe/command 条目声明一个新参数，按名称指定(可重复)",
        "為 exe/command 條目宣告一個新參數，依名稱指定(可重複)",
    ),
    row!(
        ...
        "注册执行器:skit runner add 名称 命令…({{prompt}} 标记渲染后提示词的位置;每个 shell 词就是一个参数,不经过 shell)",
        "註冊執行器:skit runner add 名稱 命令…({{prompt}} 標記渲染後提示詞的位置;每個 shell 詞就是一個參數,不經過 shell)",
    ),
    row!(
    ...
    row!(
        ...
        "跳过参数表单与注入,原样运行脚本(逃生门)",
        "跳過參數表單與注入,原樣執行腳本(逃生門)",
    ),
    row!("Remove every dependency", "清空所有依赖", "清空所有依賴",),
    row!(
    ...
    row!(
        ...
        "在本次运行前忘掉记住的额外参数(未传入任何参数时,原本会沿用它们)",
        "在本次執行前忘掉記住的額外參數(未傳入任何參數時,原本會沿用它們)",
    ),
    row!("Configuration key", "配置键", "組態鍵"),
    row!("Confirm deletion", "确认删除", "確認刪除"),
    row!("Skip confirmation", "跳过确认", "跳過確認",),
    row!(
        "Save the last run's values without asking (automation-friendly)",
        "直接存上次运行的值，不再询问（适合自动化）",
        "直接存上次執行的值，不再詢問（適合自動化）",
    ),
    row!(
        "Delete a named preset from an entry",
        "从条目删除一个具名参数组合（preset）",
        "從條目刪除一個具名參數組合（preset）",
    ),
    row!(
    ...
    row!(
        ...
        "仅提示词:完全不做变量插入——正文原封不动送达",
        "僅提示詞:完全不做變量插入——內文原封不動送達",
    ),
    row!(
    ...
    row!("Output as JSON", "以 JSON 输出", "以 JSON 輸出",),
    row!(
    ...
    row!(
        ...
        "仅提示词:开关此提示词的变量插入(关闭 = 正文原封不动送达)",
        "僅提示詞:開關此提示詞的變量插入(關閉 = 內文原封不動送達)",
    ),
    row!(
    ...
    row!(
        ...
        "安装到这个 skills 目录,而不是具名目标",
        "安裝到這個 skills 目錄,而不是具名目標",
    ),
    row!(
    ...
    row!(
        ...
        "把 skit 的 Agent Skill 安装到 AI agent 的 skills 目录",
        "把 skit 的 Agent Skill 安裝到 AI agent 的 skills 目錄",
    ),
    row!(
        "List the configured runners (seeds them into config on first use)",
        "列出已配置的执行器(首次使用时会写入配置文件)",
        "列出已設定的執行器(首次使用時會寫入設定檔)",
    ),
    row!(
    ...
    row!(
        ...
        "仅限 Shell:把常量改写成 ${NAME:-default} 写法(改的是仓库副本),之后它的值就用环境变量传递,不必再改写临时副本(可重复)",
        "僅限 Shell:把常數改寫成 ${NAME:-default} 寫法(改的是倉庫副本),之後它的值就用環境變數傳遞,不必再改寫臨時副本(可重複)",
    ),
    row!(
        "Force the language kind (e.g. shell, js) for an extensionless file",
        "强制指定语言类型（如 shell、js），用于没有扩展名的文件",
        "強制指定語言類型（如 shell、js），用於沒有副檔名的檔案",
    ),
    row!(
    ...
    row!(
        ...
        "固定提示词条目使用的执行器(见 skit runner list)",
        "釘選提示詞項目使用的執行器(見 skit runner list)",
    ),
    row!(
        "Prompt only: pin the agent this prompt runs with (empty value clears the pin)",
        "仅提示词:固定此提示词使用的执行器(空值清除固定)",
        "僅提示詞:釘選此提示詞使用的執行器(空值清除釘選)",
    ),
    row!(
        "Pin the interpreter/runtime an interpreted entry runs with (e.g. zsh, bun; empty value returns to automatic)",
        "钉选解释型条目使用的解释器/运行时(例如 zsh、bun;留空即回到自动检测)",
        "釘選直譯型項目使用的直譯器/執行環境(例如 zsh、bun;留空即回到自動偵測)",
    ),
    row!(
    ...
    row!(
        ...
        "打印实际会运行的完整命令（token 与通配符已展开）后退出",
        "印出實際會執行的完整命令（token 與萬用字元已展開）後結束",
    ),
    row!(
    ...
    row!(
        ...
        "仅限 command:改写模板({placeholders} 会从中重新读取)",
        "僅限 command:改寫模板({placeholders} 會從中重新讀取)",
    ),
    row!(
    ...
    row!(
        ...
        "条目在 PATH 上所需的外部命令（可重复；替换整个列表）",
        "條目在 PATH 上所需的外部命令（可重複；取代整份清單）",
    ),
    row!(
    ...
    row!(
        ...
        "设置条目的运行位置:origin(它自己的文件夹)、store、invoke(你运行 skit 的位置),或一个绝对路径",
        "設定項目的執行位置:origin(它自己的資料夾)、store、invoke(你執行 skit 的位置),或一個絕對路徑",
    ),
    ...
    row!(
        ...
        "把一组参数值保存成参数组合(preset)",
        "把一組參數值儲存成參數組合(preset)",
    ),
    row!(
    ...
    row!(
        ...
        "以此执行器运行提示词条目(仅本次运行,覆盖其固定设置)",
        "以此執行器執行提示詞項目(僅本次執行,覆蓋其釘選)",
    ),
    row!(
        "Set a declared parameter's default, as NAME=VALUE",
        "设置声明参数的默认值，格式 NAME=VALUE",
        "設定宣告參數的預設值，格式 NAME=VALUE",
    ),
    row!(
        "Set a declared flag parameter's option, as NAME=--out (empty = positional)",
        "设置声明的 flag 参数选项，格式 NAME=--out(留空为位置参数)",
        "設定宣告的 flag 參數選項，格式 NAME=--out(留空為位置參數)",
    ),
    row!(
        "Set a parameter's form prompt, as NAME=text (repeatable)",
        "以 NAME=text 设定参数的表单提示(可重复)",
        "以 NAME=text 設定參數的表單提示(可重複)",
    ),
    row!(
        "Set a declared parameter's type, as NAME=str|int|float|bool|choice|path",
        "设置声明参数的类型，格式 NAME=str|int|float|bool|choice|path",
        "設定宣告參數的類型，格式 NAME=str|int|float|bool|choice|path",
    ),
    row!(
        "Read a secret parameter from an environment variable at run time, as NAME=ENVVAR (empty ENVVAR clears it; repeatable)",
        "运行时从环境变量读取机密参数，格式 NAME=ENVVAR（ENVVAR 留空表示清除；可重复）",
        "執行時從環境變數讀取機密參數，格式 NAME=ENVVAR（ENVVAR 留空表示清除；可重複）",
    ),
    row!(
        "Set a declared parameter's choices, as NAME=a,b,c (comma separated)",
        "设置声明参数的可选值，格式 NAME=a,b,c(逗号分隔)",
        "設定宣告參數的可選值，格式 NAME=a,b,c(逗號分隔)",
    ),
    row!(
        "Set how a declared parameter reaches the program, as NAME=env|flag|placeholder",
        "设置声明参数如何传递给程序，格式 NAME=env|flag|placeholder",
        "設定宣告參數如何傳遞給程式，格式 NAME=env|flag|placeholder",
    ),
    row!(
        "Set a declared parameter's help text, as NAME=text",
        "设置声明参数的帮助文字，格式 NAME=text",
        "設定宣告參數的說明文字，格式 NAME=text",
    ),
    row!(
        ...
        "直接以 NAME=VALUE 指定参数值(可重复;值可用 {cwd}、{env:VAR} 等占位符;已指定的字段表单不再询问)",
        "直接以 NAME=VALUE 指定參數值(可重複;值可用 {cwd}、{env:VAR} 等代換符;已指定的欄位表單不再詢問)",
    ),
    row!(
    ...
    row!(
        ...
        "將檔案加入為 AI agent 的提示詞(不給路徑則在編輯器中撰寫新的)",
    ),
    row!(
        "Install into the current project (./.claude, ./.codex) instead of your home directory",
        "安装到当前项目(./.claude、./.codex),而不是主目录",
        "安裝到目前專案(./.claude、./.codex),而不是家目錄",
    ),
    row!(
        "Write a brand-new script in your editor, then add it",
        "在编辑器里新写一支脚本,再加入 skit",
        "在編輯器裡新寫一支腳本,再加入 skit",
    ),
    row!(
    ...
    row!("Accent color", "强调色", "強調色"),
    row!(
        "The color of borders, markers, and keys in Terminal colors. Red, green, and yellow also show errors, success, and warnings.",
        "「终端配色」下边框、标记和按键用的颜色。红、绿、黄也用来表示错误、成功和警告。",
        "「終端機配色」下邊框、標記和按鍵用的顏色。紅、綠、黃也用來表示錯誤、成功和警告。",
    ),
    row!(
        "Automatic — cyan on a dark background, magenta on a light one",
        "自动——深色背景用青色，浅色背景用洋红",
        "自動——深色背景用青色，淺色背景用洋紅",
    ),
    row!("None — no color", "无——不上色", "無——不上色"),
    row!("Red", "红色", "紅色"),
    row!("Green", "绿色", "綠色"),
    row!("Yellow", "黄色", "黃色"),
    row!("Blue", "蓝色", "藍色"),
    row!("Magenta", "洋红", "洋紅"),
    row!("Cyan", "青色", "青色"),
    row!("Bright red", "亮红色", "亮紅色"),
    row!("Bright green", "亮绿色", "亮綠色"),
    row!("Bright yellow", "亮黄色", "亮黃色"),
    row!("Bright blue", "亮蓝色", "亮藍色"),
    row!("Bright magenta", "亮洋红", "亮洋紅"),
    row!("Bright cyan", "亮青色", "亮青色"),
    row!(
