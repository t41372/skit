...
use skit_i18n::{
    Locale, available_locale_tags, catalog, detect_locale, text,
};
...
fn assert_catalog_parity(locale: Locale) {
    ...
    assert_eq!(translated, english, "{locale:?} catalog ids drifted from English");
}
...
fn test_pseudo_preserves_placeholder() {
    ...
    assert!(rendered.replace("%(file)s", "photo.py").contains("photo.py"));
}
