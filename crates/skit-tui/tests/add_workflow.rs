...
fn add_footer_labels_and_select_options_follow_the_review_lane() {
    for (state, edit_label, reference_label) in [
        ...
        let mut session = AddScreenSession::default();
        let (mut terminal, mut geometry) = draw_add(&mut session, &state);
        for _ in 0..2 {
            assert_eq!(
                session.handle_event(
                    Event::Key(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE)),
            ...
            (terminal, geometry) = draw_add(&mut session, &state);
        }
        assert!(
...
fn add_footer_wraps_only_past_its_exact_measured_width() {
    ...
    for (width, all_visible) in [
        ...
        if width == 1 {
            assert!(
                footer_hits.is_empty(),
                "a one-cell fragment must not advertise an incomplete action"
            );
        } else if !all_visible {
            assert_eq!(
