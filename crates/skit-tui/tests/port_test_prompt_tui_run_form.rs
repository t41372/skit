...
fn draw(session:&mut TuiSession,state:&LibraryState,width:u16,height:u16)->(Terminal<TestBackend>,ViewGeometry){let mut terminal=Terminal::new(TestBackend::new(width,height)).unwrap();let mut geometry=ViewGeometry::default();terminal.draw(|frame|geometry=render_with_session(frame,state,Locale::En,session)).unwrap();(terminal,geometry)}
...
fn drive(session:&mut TuiSession,state:&mut LibraryState,geometry:&ViewGeometry,event:Event)->EventHandling{let handling=session.handle_event(event,state,geometry);if let EventHandling::Action(action)=&handling{let _=state.update(action.clone());}handling}
...
fn test_form_picker_mouse_click_picks_a_runner(){
 let mut state=state_with_form(form(&[ParamDecl::new("a")],&["claude","codex"],"claude"));state.update(Action::FocusField(0));let mut session=TuiSession::default();let(_,geometry)=draw(&mut session,&state,90,24);drive(&mut session,&mut state,&geometry,key(KeyCode::Enter));
 let(terminal,geometry)=draw(&mut session,&state,90,24);let rendered=buffer_text(terminal.backend().buffer());assert!(rendered.contains("codex"),"runner option not rendered: {rendered}");let row=row_containing(terminal.backend().buffer(),"codex");drive(&mut session,&mut state,&geometry,mouse(8,row));
 assert_eq!(state.run_form().unwrap().fields()[0].control.value(),"codex");
}
...
fn test_pinned_promptless_prompt_keeps_the_shortcut(){
 let mut state=state_with_form(form(&[],&["claude","codex"],"claude"));
 let run=state.run_form().unwrap();assert!(!run.has_parameters());assert!(run.has_runner_picker());assert_eq!(run.fields()[0].control.value(),"claude");
 let mut session=TuiSession::default();let(terminal,_)=draw(&mut session,&state,90,20);let rendered=buffer_text(terminal.backend().buffer());assert!(rendered.contains("Runner")&&rendered.contains("claude"),"pinned promptless prompt skipped or hid its visible runner form: {rendered}");
 assert_eq!(submitted_runner(state.update(Action::Submit)),"claude","Enter-only promptless launch did not carry the pin");
}

#[test]
fn test_stale_pin_cannot_block_run_form_override(){
 let mut state=state_with_form(form(&[ParamDecl::new("a")],&["working"],"removed"));
 let runner=&state.run_form().unwrap().fields()[0];assert_eq!(runner.control.value(),"working","a stale pin that is no longer configured remained the form default");
 let FormControl::Choice(choice)=&runner.control else{panic!("prompt runner stopped being a picker")};assert_eq!(choice.options,["working"]);
 state.update(Action::SetFieldValue{field:1,value:"hello".to_owned()});
 assert_eq!(submitted_runner(state.update(Action::Submit)),"working");
}
