...
fn assert_stale_footer_is_blocked(
    session: &TuiSession,
    state: &LibraryState,
    geometry: &ViewGeometry,
    underlay_outcome: &EventHandling,
) {
    let mut overlay_probe = session.try_fork().unwrap();
    let (column, row) = footer_target(geometry);
    let overlay_outcome = primary_click(&mut overlay_probe, state, geometry, column, row);
    assert_ne!(
        &overlay_outcome, underlay_outcome,
        "a blocking overlay leaked a stale global-footer target",
    );
    assert_ne!(
        overlay_outcome,
        EventHandling::Ignored,
        "a blocking overlay did not own a stale global-footer coordinate",
    );
}
...
fn settings_prompt_picker_blocks_global_footer_but_keeps_its_owner() {
    ...
    let (_, stale_geometry) = draw(&mut settings_session, &settings_state, 90, 32);
    let mut underlay_probe = settings_session.try_fork().unwrap();
    let (footer_column, footer_row) = footer_target(&stale_geometry);
    let underlay_outcome = primary_click(
        &mut underlay_probe,
        &settings_state,
        &stale_geometry,
        footer_column,
    ...
    assert!(matches!(underlay_outcome, EventHandling::Action(_)));
    assert_eq!(
    ...
    assert!(
        !geometry
            ...
            .iter()
            .any(|hit| matches!(hit.action, HitTarget::Command(_))),
        "the active Settings picker published the blocked shared footer",
    );
    assert_stale_footer_is_blocked(
        &settings_session,
        &settings_state,
        &stale_geometry,
        &underlay_outcome,
    ...
    assert_eq!(
        ...
        EventHandling::Action(Action::Settings(
            skit_ui::SettingsAction::SetPromptCandidates {
                selected: vec!["VALUE_1".to_owned()],
            },
        )),
...
fn agent_picker_blocks_global_footer_but_keeps_its_owner() {
    ...
    let (_, stale_geometry) = draw(&mut preferences_session, &preferences_state, 100, 30);
    let mut underlay_probe = preferences_session.try_fork().unwrap();
    let (footer_column, footer_row) = footer_target(&stale_geometry);
    let underlay_outcome = primary_click(
        &mut underlay_probe,
        &preferences_state,
    ...
    assert!(matches!(underlay_outcome, EventHandling::Action(_)));
    preferences_state.update(Action::Preferences(
    ...
    assert!(
        !geometry
            ...
            .iter()
            .any(|hit| matches!(hit.action, HitTarget::Command(_))),
        "the active agent picker published the blocked shared footer",
    );
    assert_stale_footer_is_blocked(
        &preferences_session,
        &preferences_state,
...
fn runner_editor_blocks_the_underlying_run_field_command_hit() {
    ...
    let (_, stale_geometry) = draw(&mut session, &state, 100, 32);
    let underlay = stale_geometry
        .hits
        .iter()
        .find(|hit| matches!(hit.action, HitTarget::RunFieldCommand { .. }))
        .expect("the Run form exposes one field command before RunnerEditor opens")
        .rect;
    state.update(Action::OpenRunRunnerEditor);
    let (terminal, geometry) = draw(&mut session, &state, 100, 32);
    assert!(
        !geometry
            .hits
            .iter()
            .any(|hit| matches!(hit.action, HitTarget::RunFieldCommand { .. })),
        "the active RunnerEditor published an underlying Run command",
    );
    assert_eq!(
        session.handle_event(
            mouse(
    ...
    let (save_column, save_row) = buffer_position(terminal.backend().buffer(), "Enter Save");
    assert_eq!(
        session.handle_event(
            mouse(
                MouseEventKind::Down(MouseButton::Left),
    ...
    assert_eq!(
        session.handle_event(
            mouse(MouseEventKind::Up(MouseButton::Left), save_column, save_row,),
            &state,
            &geometry,
        ),
        EventHandling::Action(Action::RunnerEditor(RunnerEditorAction::Submit)),
        "the RunnerEditor lost its own positive Save click",
    );
    assert!(matches!(
        session.handle_event(
            key(KeyCode::Char('x'), KeyModifiers::NONE),
...
fn resize_cancels_an_armed_add_file_overlay_row_without_disabling_it() {
    ...
    assert!(matches!(
        ...
        EventHandling::Action(Action::Add(AddAction::PickedSourcePath(path)))
