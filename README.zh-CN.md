...
- **收纳脚本与提示词**。`skit add` 把散落各处的脚本与提示词收进同一个可搜索的库——复制一份收进库里，或用引用模式直接指向原始文件。不确定要加什么？直接跑 `skit add`，它会问你；丢一个它认不出种类的文件给它，它也是问你，而不是甩一句错误了事。
- **参数不再折磨人**。命令行参数、`input()`、你勾选管理的常量，全部变成表单字段（choices 变选择器、布尔变复选框、路径会边打边补全还能开文件浏览器、类型自动把关）。
- **路径不用手打**。每个文本字段旁边都有一个 `📁 浏览`，点下去就是文件选择器；路径字段还会打到一半就提示补全，按 `→` 直接采用。从此不用手打目录。（键盘走 `Ctrl+T` →〈文件或文件夹…〉，开的是同一个选择器。）
- **它会记住**。脚本默认值直接填进表单，脚本改了也跟着变；上次填的值自动带回，`↺ 默认值`（Ctrl+O）一键改回脚本自己的值。常用的一组存成命名组合。标记为机密的参数永不保存——记住的值、组合、运行历史里都不会有它。`{cwd}`、`{today}` 这类值 token 让同一个组合跨机器、跨目录通用。
...
- **提示词是库中的一等条目**。存一份带参数的提示词（交给 skit 管理的 `{{占位符}}` 会变成表单字段），一键丢给你的 coding agent——claude、codex、opencode 等执行器已预先配置好，其他 CLI 一句 `skit runner add` 就能接上。
- **也是你的 agent 的工具库**。官方 [Agent Skill](https://agentskills.io) 教会 Claude Code、Codex、Cursor、Gemini CLI 等 agent 完整用法：用 `skit list` 探索条目、用 `skit show` 读参数结构、用 `skit run --set … --no-input` 运行。一句 `skit agent install` 就装好——见[给你的 AI agent 用](#给你的-ai-agent-用)。
...
| ![工具库](https://raw.githubusercontent.com/t41372/skit/main/docs/assets/tui-library-zh.png) | ![执行表单](https://raw.githubusercontent.com/t41372/skit/main/docs/assets/tui-form-zh.png) |
|:--:|:--:|
| **工具库**——每个动作都在画面上，鼠标键盘皆可 | **执行表单**——从脚本自己的参数生成 |
...
| **加入脚本**——静态检测参数，勾选即管理 | **脚本设置**——参数、机密、组合、依赖 |
...
| **提示词**（prompt） | 你的 agent CLI（claude · codex · …） | `{{name}}` 占位符 | — | — | needs |
...
提示词条目是给 AI coding agent 的、可复用且带参数的一段文字。添加一个 `.prompt.md` 文件（或用 `skit add --prompt` 直接起草）后，交互加入时可在复核页中选择哪些检测到的 `{{占位符}}` 要变成表单字段。候选不超过 30 个时默认全选；超过 30 个时默认一个都不选，以免把代码示例误认为变量。管理中的字段完整享有组合 / 上次值 / `--set` 体验。没有任何转义规则要学：凡是你没交给 skit 管理的内容——包括没管理的 `{{占位符}}`——一律逐字节原样送达 agent；每个提示词还有一个插值总开关（`--no-interpolate`，或条目设置里的一个复选框），写作时没考虑插值的长提示词可以整个关掉。运行时 skit 渲染文字，并把它交给所选执行器配置的 argv 命令。预置的 Claude、Codex、OpenCode 和 Antigravity 命令会打开各自的交互 session；Amp 是刻意保留的例外，按其官方 `amp -x` 一次性模式执行提示词后返回。**执行器**在运行表单上选（或按提示词固定），claude / codex / opencode / amp / antigravity 已预先配置，其他工具用 `skit runner add mycli -- mycli run {{prompt}}` 注册。渲染后的提示词以单一进程参数传递——中间没有 shell。一个诚实的提醒：提示词不是保密通道——渲染后的内容会落在对方 agent 自己的 session 记录里。

```bash
skit add review.prompt.md            # 管理中的占位符变成表单字段
skit run review                      # 选 agent、填表单、出发
