...
提示词条目是给 AI coding agent 的、可复用且带参数的一段文字。添加一个 `.prompt.md` 文件（或用 `skit add --prompt` 直接起草），每个 `{占位符}` 都会成为表单字段，完整享有组合 / 上次值 / `--set` 体验；`{{` 和 `}}` 写出字面大括号，未纳管的占位符原样保留。运行时 skit 渲染文字并打开 agent 自己的交互 session——**执行器**在运行表单上选（或按提示词固定），claude / codex / opencode / amp / antigravity 已预先配置，其他工具用 `skit runner add mycli -- mycli run {prompt}` 注册。渲染后的提示词以单一进程参数传递——中间没有 shell——多行文字和奇怪字符都逐字节原样到达。一个诚实的提醒：提示词不是保密通道——渲染后的内容会落在对方 agent 自己的 session 记录里。
