...
- **提示词也是脚本**。存一份带参数的提示词(`{{占位符}}` 会变成表单字段),一键丢给你的 coding agent——claude、codex、opencode 等执行器已预先配置好,其他 CLI 一句 `skit runner add` 就能接上。
...
提示词条目是给 AI coding agent 的、可复用且带参数的一段文字。添加一个 `.prompt.md` 文件（或用 `skit add --prompt` 直接起草），每个 `{{占位符}}`（双大括号，所以塞满单大括号的代码片段不会误触发）都会成为表单字段，完整享有组合 / 上次值 / `--set` 体验。没有任何转义规则要学：凡是你没纳管的内容——包括没纳管的 `{{占位符}}`——一律逐字节原样送达 agent；每个提示词还有一个插值总开关（`--no-interpolate`，或脚本设置里的一个复选框），写作时没考虑插值的长提示词可以整个关掉。运行时 skit 渲染文字并打开 agent 自己的交互 session——**执行器**在运行表单上选（或按提示词固定），claude / codex / opencode / amp / antigravity 已预先配置，其他工具用 `skit runner add mycli -- mycli run {{prompt}}` 注册。渲染后的提示词以单一进程参数传递——中间没有 shell。一个诚实的提醒：提示词不是保密通道——渲染后的内容会落在对方 agent 自己的 session 记录里。

```bash
skit add review.prompt.md            # 占位符变成表单字段
...
默认：PyPI → 清华 / 阿里云 / 中科大；npm → npmmirror；Python 构建与 uv 二进制 → 南京大学。哪个镜像挂了，在 TUI 偏好设置（或首次运行向导）选 `custom` 就能逐项换地址。
