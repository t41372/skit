...
- **收纳脚本与提示词**。`skit add` 把散落各处的脚本与提示词收进同一个可搜索的库——复制一份收进库里，或用引用模式直接指向原始文件。
...
- **提示词是库中的一等条目**。存一份带参数的提示词（纳管的 `{{占位符}}` 会变成表单字段），一键丢给你的 coding agent——claude、codex、opencode 等执行器已预先配置好，其他 CLI 一句 `skit runner add` 就能接上。
...
提示词条目是给 AI coding agent 的、可复用且带参数的一段文字。添加一个 `.prompt.md` 文件（或用 `skit add --prompt` 直接起草）后，交互加入时可在复核页中选择哪些检测到的 `{{占位符}}` 纳管成表单字段。候选不超过 30 个时默认全选；超过 30 个时默认一个都不选，以免把代码示例误认为变量。纳管后的字段完整享有组合 / 上次值 / `--set` 体验。没有任何转义规则要学：凡是你没纳管的内容——包括没纳管的 `{{占位符}}`——一律逐字节原样送达 agent；每个提示词还有一个插值总开关（`--no-interpolate`，或条目设置里的一个复选框），写作时没考虑插值的长提示词可以整个关掉。运行时 skit 渲染文字，并把它交给所选执行器配置的 argv 命令。预置的 Claude、Codex、OpenCode 和 Antigravity 命令会打开各自的交互 session；Amp 是刻意保留的例外，按其官方 `amp -x` 一次性模式执行提示词后返回。**执行器**在运行表单上选（或按提示词固定），claude / codex / opencode / amp / antigravity 已预先配置，其他工具用 `skit runner add mycli -- mycli run {{prompt}}` 注册。渲染后的提示词以单一进程参数传递——中间没有 shell。一个诚实的提醒：提示词不是保密通道——渲染后的内容会落在对方 agent 自己的 session 记录里。

```bash
skit add review.prompt.md            # 纳管后的占位符变成表单字段
