...
- **`--no-input`** on commands that can prompt: `add`, `run`, `remove`, `edit`,
  `runner remove`, and `preset delete`. Read commands never prompt and do not take it.
- **`-y` or `--yes`** confirms `remove`, `runner remove`, and `preset delete`.
  Use this option only after the user approves the operation.
...
| Flag | Effect |
| --- | --- |
| `--data-dir PATH` | use a specific skit data directory for this command |
...
| `126` | the target exists but cannot run — missing interpreter or JS runtime, a `needs` command absent from `PATH`, no runner resolvable for a prompt |
...
exclusive (exit 2), and per-lane flags are refused with a hint when they do not
...
- `skit edit NAME` — open the stored source (the original, in reference mode) in
  ...
  exe/command — there is no editable source.
...
| `--binding NAME=const\|input\|envdefault\|none` | source binding for a declared field |
| `--flag NAME=--out` | the CLI flag to assemble (empty = positional) |
| `--multiple NAME` / `--no-multiple NAME` | allow many values or only one value |
| `--repeat NAME` / `--no-repeat NAME` | repeat the flag for each value or put all values after one flag |
| `--env-target NAME=ENVVAR` | environment variable set by `env` delivery |
| `--action NAME=ACTION` | Boolean flag action, such as `store_true` or `store_false` |
...
home. The bare form uses the directory only when it finds exactly one supported
directory. If it finds none or more than one, it requires an explicit target.
