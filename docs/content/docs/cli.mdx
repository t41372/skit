...
| `--save-preset NAME` | save this run's values as a preset (real runs only — a `--dry-run` persists nothing; to mint without running, use `preset save --set`) |
...
- `skit preset save NAME PRESET [--set K=V]... [--from-last]` — `--set` mints the
  ...
  lane); `--from-last` snapshots the last run's values instead. The two sources
  are mutually exclusive; with neither, an interactive terminal is required.
