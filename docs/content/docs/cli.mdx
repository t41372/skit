...
- **`--no-input`** on the commands that could prompt: `add`, `run`, `edit`, and
  the destructive trio `remove`, `preset delete`, `runner remove` (which confirm
  unless `--yes`). Read commands never prompt and don't take it. On `edit` it
  means *refuse*, not *proceed*: an editor session needs a terminal, and under a
  pty with nobody typing this is the only way to say there is no human.
