...
| `--save-preset NAME` | save this run's values as a preset (real runs only — a `--dry-run` persists no preset, values, or run history; to mint without running, use `preset save --set`) |
...
- `skit show NAME [--json]` — full metadata, dependencies, parameters, presets.
  ...
  Two top-level keys describe where the form came from, and `param_origin` is
  the one to read. `param_source` is the older token and keeps its historical
  values for compatibility — including `"argparse"` for *every* static CLI
  reader (Python argparse/click/typer, but also shell `getopts`, JS
  `parseArgs`, PowerShell `param()`). `param_origin` says the same thing
  honestly: `managed` (skit-managed constants/inputs), `reader` (any static
  CLI reader), `declared` (hand-declared schema), `command` (template
  placeholders — prompt bodies included). New automation should key on
  `param_origin`; `param_source` will not change meaning, but it will not get
  new values either.
...
- `skit preset save NAME PRESET [--set K=V]... [--from-last]` — `--set` mints the
  preset from explicit values without running anything (the non-interactive
  lane); like every preset writer it stores a full snapshot, so fields you don't
  name take the entry's own defaults — never this machine's history.
  `--from-last` snapshots the last run's values instead. The two sources are
  mutually exclusive; with neither, an interactive terminal is required.
...
`meta.toml`. JSON keys include `uv`, `uv_required` (whether a missing uv is a
failure here — true only when Python entries exist; the exit code follows it),
`entries`, `missing`, `drift`, `needs_missing`, `launch_blocked`,
