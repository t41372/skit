...
- **The `--` tail** (extra arguments after `--`) is remembered per entry and
  ...
  replays literally — in both the CLI and the TUI, and submitting the launch
  menu with the prefilled tail untouched keeps whichever it was. Whenever a
  literal-replay tail contains text that WOULD expand in the launch menu
  (tokens, globs, a leading `~`), skit says "passed as-is" alongside the run —
  that includes deliberately shell-quoted CLI tails (correct, just stated) and
  tails remembered by versions before this record existed; edit the tail in
  the launch menu once to re-capture it as expanding form text.
