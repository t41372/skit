...
the menu immediately. `skit params --resync` writes refreshed definitions back.
Clear an override to use the script's current value.
...
when that name is bound exactly once in the file and does not look like a secret.
...
`--set NAME=` expresses the same thing from the CLI. For types where empty cannot
...
- **Last-used values** come back prefilled on the next run. A submitted value
  ...
  script's default is not buried by an old run. To pin a value deliberately, use
