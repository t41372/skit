...
- **Last-used values** come back prefilled on the next run. A submitted value
  ...
  a preset: every preset writer stores a full snapshot verbatim, cleared fields
  included (`preset save --set` backfills unnamed fields from the entry's own
  defaults, never from history).
