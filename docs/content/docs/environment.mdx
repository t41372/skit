...
`skit doctor` prints the resolved library, config and state paths — and carries them as
`location`, `config_dir` and `state_dir` under `--json`. The TUI's Health screen shows the
same three.
