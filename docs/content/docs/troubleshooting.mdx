...
## Shell scripts do not run on Windows
...
It will not, since 0.4.0: defaults are read from the script's current source on
...
and presets — reinstall and you are back. skit never writes to your `PATH`,
shell profile, or global uv config.

**What runtimes does skit need?**
skit is a Rust binary and does not need Python. A Python entry needs uv. skit can
download a pinned, checksum-verified private uv copy after consent. For JS/TS,
