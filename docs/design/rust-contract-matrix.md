...
| --- | --- | --- |
| Metadata, open kinds, unknown TOML, exact source bytes and permissions, durable identity, and metadata/source CAS | Complete | `skit-domain/tests/contract.rs`, `skit-store/tests/mutations*.rs`, coordinated state/cleanup rollback and race owners; pinned-main store tests |
...
The current local snapshot has 21 rows: 20 complete and 1 in progress. The integrated candidate
passes 4,054 workspace tests with 0 failures and 526 classified ignores, complete executable-source
line coverage, warnings-denied Clippy and Rustdoc, all three locale catalogs, and the local
supply-chain, docs, package, demo, and benchmark gates. The remaining release row needs native
platform and declared-runtime CI, the user hands-on UI check, and an explicitly approved mutation
run; a lower-layer green test does not close it.
