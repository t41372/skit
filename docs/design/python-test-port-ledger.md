...
| test_editor.py | 50 | crates/skit-cli/tests/port_test_editor.rs + private CLI editor-resolution units | implementation parity (52 REAL / 0 ignored across 50 frozen owners + 2 Rust regressions) · all four former helper stubs now run through public CLI/process owners; copy edit passes the exact stored path and finalizes only metadata/hash under identity and complete-meta CAS; launch and post-read errors retain typed OS causes and exact three-language copy; ordinary editor rc≠0 remains successful in existing edit, true-PTY add, and TUI author/edit paths `d9a78d0`/`8d3622d` · nine resolution owners cover the shared precedence/tokenization helper and Windows dialect `cf8ebba` · pre-authoring PTY contracts remain `7fd57de` |
