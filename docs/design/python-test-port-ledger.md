...
| test_declared_params.py | 52 | crates/skit-cli/tests/port_test_declared_params.rs | in fix (43/9) · new command/prompt names outside their real placeholder sets default to env delivery `6f193e6` · metadata-schema edits print the frozen post-commit receipt after secret purge while JSON remains pure `878826b` · **empty-params plain line FIXED 8633128** · secret plaintext purge announcement fixed `0335ed1` · human secret defaults/last values masked `669abd1` · public env-source warning/no-op and secret transition order fixed `3d9c8ba` · 8 divergences (batch fault tolerance, secret/template defaults) · 1 weakening caught+fixed by verify |
...
| Python module | # | Rust target | Status |
| --- | --- | --- | --- |
| test_cli.py | 140 | crates/skit-cli/tests/port_test_cli.rs | in fix (97/43) · bare unknown directories now get the frozen noninteractive `--exe` recovery before reads or writes `0d1a294` · source-managed edits print the frozen post-commit receipt and preserve bytes for an idempotent resync `0c64921` · malformed `--prompt` tweaks warn and preserve bytes while other malformed axes stay fatal `ccec2c6` · reused-argument notice escapes user markup `0b8bf2d` · typed run values use the form voice `33d9a4a` · missing, typed-directory, and unreadable add-source preflight is fixed `22016c2`/`4fc1659`/`de754cf` · explicit passthrough arguments satisfy only blank required flags `39f9bd0` · one params drift-completeness divergence remains |
| test_prompt_cli.py | 150 | crates/skit-cli/tests/port_test_prompt_cli.rs | in fix (105/45 + 1 Rust regression) · noninteractive edit reports unmanaged placeholders with capped localized tails from the validated snapshot `c9d78af` · real PTY runner confirm/decline and two confirmation-window CAS owners folded `ef5c5a2`; runner decline/EOF now use frozen exit 1 without changing entry-removal abort 130 · five prompt-editor owners fixed `24e9510`: pre-editor name/collision validation, shared localized starter, and untouched cleanup · **stdin store-error 'taken' FIXED 592d236** · **edit drift hint FIXED 1cac4a4** · runner trim/pick persistence fixed `2c1f6ef` · Unicode `--set` fixed `07dfbce` · add-time unknown-runner voice fixed `3d43a40` · typed value voice fixed `33d9a4a` · compound `.prompt.md` naming fixed `19cc2ff` · missing prompt body exit fixed `0f1fdd9` · argv NUL/limit voices fixed `41f2ec0` · stable-name recovery quoting fixed `076d04c` · runner refusal diagnostics fixed `085a4b3` · insertion/flood summary fixed `2087afa` · blank runner name fixed `d92e264` · stdin boundary fixed `53bdb1c` · runner argv reasons fixed `e64407a` · params prompt-policy fixed `eeed834` · add-source preflight fixed `22016c2`/`4fc1659` · Markdown recovery fixed `c6d14ec` · malformed-runner recovery fixed `0e6d826` · runner-remove target validation fixed `f0dfcff` · doctor entry taxonomy and oracle translations fixed `06b4c99` · interpolation-off read notice fixed `52ef759` · schema edits while insertion is off fixed `4a8aa74` · remaining divergences are runner management/add/params voices |
...
| test_add_no_source.py | 68 | crates/skit-cli/tests/port_test_add_no_source.rs | in fix (20/48) · the bare menu shows the complete frozen choice/default prompt `659be98` · pipe/no-input lane advice fixed `157f9b7` · 10 remaining divergences belong to the full interactive selector/directory consent roots |
...
| test_add_lane_contracts.py | 21 | crates/skit-cli/tests/port_test_add_lane_contracts.rs | in fix (18/3) · prompt editor `--no-input` now has a true PTY owner and exact pipe recovery `4ba75ec`; the first successful human `--manage` names the modeled reader form set aside `7c37d80` while JSON, dynamic readers, and later edits stay silent · **read-view voices ×2 FIXED 8633128** · add-time unknown-runner early refusals fixed `3d43a40` · post-editor Python-flags voice fixed `62a7df8` · 3 divergences |
| test_dependency_command_contracts.py | 20 | crates/skit-cli/tests/port_test_dependency_command_contracts.rs | in fix (12/8) · Python-only set/clear receipts use exact post-commit three-language rows `4801539` · exact shared JS Python-constraint voice fixed `ceece3b` · 5 divergences + 3 closures `ab901c4` |
...
| test_tui_nav.py | 5 | crates/skit-tui/tests/port_test_tui_nav.rs | in fix (3/2) · Add Review has true Up/Down and Tab/Shift+Tab navigation with bidirectional mouse footer targets `8eb9b03`; Preferences exposes the same commands through the real TUI event fallback while controls retain their keys `afeae66` · Add Source remains blocked by BrowseSource keyboard ownership · Settings has a multiline boundary semantic conflict |
