...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs | done (49) · 115 cross-crate/absent · 14 divergences (manifest name-key, staging cwd, stamp path, sweep) · input-order/empty-requirement fixed `54bf6c6` · mirror empty-value fixed `c3e02b8` · reference implicit scan fixed `6aedb9b` · installer argv/fallback fixed `05e8b9d` · reference refusal voices fixed `dc770fc` · 6 verifier-caught defects fixed |
...
| test_add_validation_contracts.py | 31 | crates/skit-cli/tests/port_test_add_validation_contracts.rs | done (16) · **PEP 440/508 wording ×2 FIXED c5e84ea** · explicit `--python` normalization fixed `f8f80f5` · 13 divergences (drafts-boundary guard absent #15, editor-validates-after-open, kind_for_draft missing, error wording) + 2 absent (interactive re-ask) · 1 weakening fixed |
