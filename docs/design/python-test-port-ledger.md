...
| test_store_fix.py | 38 | crates/skit-store/tests/port_test_store_fix_metadata.rs + crates/skit-store/tests/port_test_store_fix_filesystem.rs + store private atomic/lock/mutation owners + owning application/CLI targets | in progress (19 exact executable + 6 semantic closures = 25/38 accounted; 13 remain) · Metadata Batch 1 activates 12 exact corruption owners across scan, resolve, doctor rebuild, and three-locale diagnostics while preserving valid siblings, open kinds, unknown TOML, and metadata bytes · Batch 2 activates four exact filesystem/recovery/atomic owners: a lost index cannot cause slug overwrite, a corrupt orphan stays isolated and reserved, a corrupt index moves byte-exact to `.corrupt` on read, and a real partial payload-write failure leaves no entry or staged bytes and permits a clean retry · Lock/concurrency Batch 4 activates three exact owners: a held persistent native registry lock blocks a checkpointed second holder, the versioned native lock persists across holders (same inode on Unix; sentinel/presence on Windows), and eight barrier-synchronized Python creates commit distinct slugs/payloads without staging residue |
