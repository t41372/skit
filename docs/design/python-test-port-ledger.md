...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs + skit-runtime private transaction owners + crates/skit-store/tests/mutations.rs | **Phase 4 implementation in progress (66/78 in the CLI owning target after cleanup rehomes)** · installer failures capture typed exit/stderr and keep rollback atomic `b1ad9ac`; removal wait/refusal owners live at the store lock boundary `a219125`; the 35 ABSENT rows were audited as 5 with stronger executable owners, 13 stale owners needing rehome/activation, and 17 real gaps · six cleanup owners now exercise real unlink/rmtree/symlink races, NotFound success, loud locked failures, partial-delete quarantine, self-heal, and exact three-language messages (`13c04fe`) · 29 ABSENT rows remain across cheap freshness/TUI preflight, every-launch sweep, diagnostics/messages/announce, and stale owner rehomes |
