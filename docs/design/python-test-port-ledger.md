...
| test_reset_default_ui.py | 14 | crates/skit-cli/tests/terminal_pty.rs + crates/skit-tui/tests/port_test_reset_default_ui.rs | implementation parity (11/3) · live source defaults are reconciled only into display clones, use source-only Python repr, and are owned by the real CLI/TUI host `fa1e9f3` |
...
| test_benchmarks_tooling.py | 156 | `skit-benchmarks` native owners; 120 `#[test]` bodies after the dataset-cardinality and repository/report batches | implementation parity audit in progress (10 exact active + 113 stronger consolidated owners + 6 stale rehomes + 0 product gaps + 5 rejected PR fixtures + 22 structured closures = 156) · ordinary and run-over generation use the same final authoritative store count policy, refuse a deterministic 2-of-3 library, keep the failed root for inspection, and do not publish a manifest · Repository/report Batch 2 activates seven stale owners against real checked-in artifacts and closes the corrupt `run.json` product gap through the public summary front door · Criterion and CodSpeed are additional measurements, not Python tooling substitutes |
