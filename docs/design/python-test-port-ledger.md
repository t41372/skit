...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs + crates/skit-store/tests/mutations.rs | done (66) · 86 architecture/cross-crate/absent · removal wait/refusal owners moved to the real store lock boundary `a219125` with persistent-inode and byte-exact no-write assertions · all 27 implementation divergences fixed through `219a136` · exact manifest/order/parser, root-cwd install, SHA-256 node_modules marker, fresh short-circuit, resolve-before-touch, persistent lock, and transactional rollback all covered · 6 verifier-caught defects fixed |
