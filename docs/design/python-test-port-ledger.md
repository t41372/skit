...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs + crates/skit-store/tests/mutations.rs | implementation parity (65/85 in the CLI owning target) · installer failures now capture typed exit/stderr, retain the actionable trusted cause, filter fixed npm noise, and preserve every authoritative byte on rollback `b1ad9ac` · removal wait/refusal owners moved to the real store lock boundary `a219125` with persistent-inode and byte-exact no-write assertions · all 27 earlier implementation divergences fixed through `219a136` · exact manifest/order/parser, root-cwd install, SHA-256 node_modules marker, fresh short-circuit, resolve-before-touch, persistent lock, and transactional rollback all covered · 7 verifier-caught defects fixed |
| test_interpreters.py | 74 | crates/skit-runtime/tests/port_test_interpreters.rs + crates/skit-cli/tests/port_test_dependency_command_contracts.rs | done (20) launch/invoke/dependency-needs half · needs set/clear exact owners moved to the real CLI receipt seam `744b3c4` · **bun run + JS refusal FIXED 19e5ab8** · 54 cross-crate · DETECTION half (shebang_program/infer_kind) OWED at skit-language |
...
| test_editor.py | 50 | crates/skit-cli/tests/port_test_editor.rs + private CLI editor-resolution units | implementation parity (47/4 across 50 frozen owners + 1 Rust regression) · nine resolution owners now exercise one production precedence/tokenization helper, including the Windows quoting and backslash dialect `cf8ebba` · add-editor requires both terminal streams, asks/validates a missing name before authoring, and uses true PTY siblings `7fd57de` · name conflict fixed `592d236` · editor resolution/Saved/reference guard fixed `1cac4a4` · post-editor flags `62a7df8`, decline `7c3d615`, early conflicts `ea4389c`, unknown shebang `7555a0e`, missing copy `add3c47`, untouched drafts `7f7be0d`, source-management refusal `4746c36` |
...
| test_add_validation_contracts.py | 31 | crates/skit-cli/tests/port_test_add_validation_contracts.rs | implementation parity (25 REAL / 4 owned-draft semantic-duplicate closures / 2 unrelated absent closures) · owned-draft boundary voices/no-write centralized `98e75e9` · PEP wording `c5e84ea`/`8cf75fe` · Python normalization `f8f80f5` · unknown shebang `c134c80` · editor flag preflight `ea80ec4` |
...
| test_add_feedback_contracts.py | 16 | crates/skit-cli/tests/port_test_add_feedback_contracts.rs | implementation parity (9 REAL / 2 owned-draft semantic duplicates / 3 absent / 2 cross-crate closures) · prompt single/compound post-commit consume and exact stored bytes `7aab925` · Python pin `affa5b2` · unknown/generic recovery `c134c80`/`7c4cb53` |
| test_edit.py | 14 | crates/skit-cli/tests/port_test_edit.rs + skit-domain source-management units | implementation parity (7/7 frozen owners plus Rust support) · candidate management now has one public typed domain result; already-managed/not-candidate requests warn in order while valid siblings commit once, JSON stays pure, secrets purge, and invalid-only batches write nothing `85ecf69` · malformed `--prompt` warns without touching source/meta/state `ccec2c6` · command-source editor contract activated `5d0d888` · reference source-management refusal exit fixed `4746c36` · remaining direct pure-helper owners cover the larger combined resync/remove/tweak API that is still split across typed owners |
...
| Python module | # | Rust target | Status |
| --- | --- | --- | --- |
| test_prompt_tui.py | 83 | crates/skit-tui/tests/port_test_prompt_tui.rs + crates/skit-cli/tests/terminal_pty.rs | done (54) · selected-runner ProgramNotFound now returns to Library without child/state writes `0f76635`; zero-runner prompts open the shared runner editor and cancel/save/failure paths match the oracle `949a51a` · real MouseDown runner selection strengthened and pinned/unpinned rerun moved to the live TUI host `f8091da` with child markers and last-value replay · **Library title FIXED 5574ff1** (工具库/庫) · remaining cross-crate run routing · Ctrl+O/Ctrl+E divergences in fix pass |
| test_path_tui.py | 61 | crates/skit-tui/tests/port_test_path_tui.rs | implementation parity (32/29) · current-directory pinned selection now has real keys, mouse, filter/empty behavior, exact three-language label, and a root `.` observable `9718a1a` · PageUp/PageDown navigation fixed `98b6bc8` · Python-compatible picked-path glob escaping fixed `0fd6d85` · remaining ignores are private/cross-crate/architecture classifications |
