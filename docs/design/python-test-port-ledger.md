...
| test_declared_params.py | 52 | crates/skit-cli/tests/port_test_declared_params.rs | in fix (42/10) · metadata-schema edits now print the frozen post-commit receipt after secret purge while JSON remains pure `878826b` · **empty-params plain line FIXED 8633128** · secret plaintext purge announcement fixed `0335ed1` · human secret defaults/last values masked `669abd1` · public env-source warning/no-op and secret transition order fixed `3d9c8ba` · 9 divergences (batch fault tolerance, secret/template defaults) · 1 weakening caught+fixed by verify |
...
| Python module | # | Rust target | Status |
| --- | --- | --- | --- |
| test_flows.py | 102 | crates/skit-application/tests/port_test_flows.rs + crates/skit-ui/tests/run_screen.rs + crates/skit-store/tests/glob_expander.rs | done (63) · 39 cross-crate · three inject-field projection owners moved to the real `RunFormView::from_declarations` seam `f5423d4` with typed default/control/secret/env/binding assertions · two filesystem-glob owners moved to the real sorted/native store adapter `0e9f082` · remaining plan_for_entry/execute/transparency → skit-cli and exact messages → skit-tui · divergences kept |
