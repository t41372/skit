...
| test_reconcile.py | 27 | crates/skit-language/tests/port_test_reconcile.rs | partial: 14 executable · 13 deferred |
...
| test_tokens.py | 21 | skit-application / skit-cli | done |
...
| test_hermeticity.py | 1 | crates/skit-cli/tests/port_test_hermeticity.rs | done |
...
### test_reconcile.py → port_test_reconcile.rs (partial: 14 executable · 13 deferred)

14 ported / 14 passed / 0 failed / 13 explicitly deferred. The deferred tests are recorded in the
machine map and no longer exist as ignored stubs, so neither Cargo nor this ledger can misreport them
as completed. The 14 that exercise the reconcile report all pass: the Rust `ReconcileReport` is fully as rich as the Python one
...
something" guard). A `skit-language` integration test cannot reach it without reversing crate dependencies. Those
