...
| test_uvman.py | 36 | crates/skit-runtime private/integration owners + existing CLI private/PTY/workflow owners | implementation parity (27 active exact / 9 classified or owed) · checksum diagnostics, directory-sync swallow, and typed unpinned refusal fixed in `7977386` · orphan-pin completeness is active in `uv::private_tests::checksum_table_pins_exactly_the_producible_triples` (`76aaab1`) · CLI Batch A rehomes seven exact consent/mirror owners through the real binary, FileConfigStore, production answer policy, and PTY · Musl Batch B moves three exact probes to the private path seam: present linker is true, unrelated contents are false, and a missing root is false; the host wrapper still supplies only the real `/lib` on Linux · the remaining 9 are 3 external/platform gates, 2 stronger semantic closures, and 4 durability private-seam rehomes |
