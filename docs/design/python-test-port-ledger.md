...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs + skit-runtime private transaction owners + crates/skit-store/tests/mutations.rs | **Phase 4 implementation in progress (65/85 in the CLI owning target before rehomes)** · installer failures capture typed exit/stderr and keep rollback atomic `b1ad9ac`; removal wait/refusal owners live at the store lock boundary `a219125`; the 35 ABSENT rows were audited as 5 with stronger executable owners, 13 stale owners needing rehome/activation, and 17 real gaps across cleanup, cheap freshness/TUI preflight, every-launch sweep, diagnostics, exact messages, and announce behavior · cleanup transaction is the first implementation batch |
...
| test_edit.py | 14 | crates/skit-cli/tests/port_test_edit.rs + skit-language source editor + skit-domain declarations | **Phase 4 parser-backed resync implementation in progress (7/7 before rehomes)** · candidate management has one typed result and partial warning behavior `85ecf69`, but the audit proved current CLI-private name matching can erase every managed row when source parsing fails, omits dropped/skipped/rebound warnings, keeps a secret literal default, and rejects an unknown tweak before a valid sibling · the coherent batch must activate all 7 direct owners plus 10 adjacent language reconcile owners and 2 form resync owners through one CLI/TUI operation · malformed `--prompt` no-write `ccec2c6`; command-source editor `5d0d888`; reference refusal `4746c36` |
