...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs | done (57) · 115 cross-crate/absent · 7 divergences (full manifest/staging cwd/stamp path, add refusal voices) · input-order/empty-requirement fixed `54bf6c6` · mirror empty-value fixed `c3e02b8` · reference implicit scan fixed `6aedb9b` · installer argv/fallback fixed `05e8b9d` · reference refusal voices fixed `dc770fc` · transactional artifact cleanup and aged injected sweep fixed `cc0ead1` · minimal dependency-free module manifests fixed `d7575c5` · 6 verifier-caught defects fixed |
