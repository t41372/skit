...
| test_declared_params.py | 52 | crates/skit-cli/tests/port_test_declared_params.rs + skit-form/store owners | in fix (48 REAL / 2 divergence / 2 closure) · implicit `params` names are separate from explicit `parameters` rows across CLI/UI/Settings `7510454` · PowerShell static reader fields merge Flag/Env riders at the form seam `0c847ea` · store roundtrip rehomed `ae955c3`; raw/typed row split closed `8f0c038` · env delivery `6f193e6`; receipts `878826b`; masking `669abd1`; env-source order `3d9c8ba` · remaining divergence is the shared declared-edit warning engine |
...
| test_add_no_source.py | 68 | crates/skit-cli/tests/port_test_add_no_source.rs | in fix (23/45) · interactive directory consent is default-yes and stores an executable reference without touching the external directory `046bd88` · bare menu prompt `659be98` · pipe/no-input advice `157f9b7` · 8 remaining divergences form the full unknown-kind selector root |
...
| test_add_lane_contracts.py | 21 | crates/skit-cli/tests/port_test_add_lane_contracts.rs | in fix (19/2) · centralized Add dispatch conflict priority fixed `1755354` · prompt editor no-input `4ba75ec` · first managed-reader notice `7c37d80` · read views `8633128` · unknown runner `3d43a40` · post-editor flags `62a7df8` · 2 owned-draft divergences remain |
| test_dependency_command_contracts.py | 20 | crates/skit-cli/tests/port_test_dependency_command_contracts.rs | in fix (15/5) · full Dependencies/Python/Needs post-commit receipt engine and needs owner rehome `744b3c4` · Python-only receipts `4801539` · JS Python-constraint voice `ceece3b` · 2 owned-draft divergences + 3 closures `ab901c4` |
