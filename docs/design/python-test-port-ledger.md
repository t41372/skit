...
| test_uvman.py | 36 | crates/skit-runtime/tests/port_test_uvman.rs + existing CLI private/PTY/workflow owners | implementation parity (24 active exact / 12 classified or owed) · checksum diagnostics, directory-sync swallow, and typed unpinned refusal fixed in `7977386` · orphan-pin completeness is active in `uv::private_tests::checksum_table_pins_exactly_the_producible_triples` (`76aaab1`) · CLI Batch A rehomes seven exact owners: non-TTY consent and configured/default/blank mirror selection run through the real binary and FileConfigStore, answer parsing runs at the private production policy, and decline/EOF run through real PTYs with exact guidance and authoritative no-write checks · the remaining 12 are 3 external/platform gates, 2 stronger semantic closures, and 7 musl/durability private-seam rehomes |
