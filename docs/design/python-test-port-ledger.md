...
| test_uvman.py | 36 | crates/skit-runtime private/integration owners + existing CLI private/PTY/workflow owners | implementation parity (31 executable exact / 3 external or platform gates / 2 stronger semantic closures; final manifest owed) · checksum diagnostics, directory-sync swallow, and typed unpinned refusal fixed in `7977386`; orphan-pin completeness is active in `76aaab1` · CLI Batch A rehomes seven exact consent/mirror owners through the real binary, FileConfigStore, production answer policy, and PTY · Musl Batch B moves three exact probes to the private path seam while the Linux host wrapper still supplies only `/lib` · Durability Batch C moves six exact owners to the actual install-operation seam: partial staged writes, staged-sync and atomic-replace failures leave no final or staged file; retry and the complete locked ensure/fetch/checksum/archive path self-heal; staged sync precedes replace; and a directory-sync failure after commit stays best-effort |
...
| test_shim.py | 38 | language semantic owners + CLI runtime owners + exact manifests | in progress: Batch A has 33 unique executable owners (18 portable + 15 production-resolved real-Python runtime/compile), 2 structured helper/ownership closures, and 3 pending CLI staged-copy owners; no product gap is known |
