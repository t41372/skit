...
| test_benchmarks_tooling.py | 156 | `skit-benchmarks` native owners; 112 `#[test]` bodies after the dataset-cardinality batch | implementation parity audit in progress (2 exact active + 113 stronger consolidated owners + 13 stale rehomes + 1 product gap + 5 rejected PR fixtures + 22 structured closures = 156) · ordinary and run-over generation now use the same final authoritative store count policy, refuse a deterministic 2-of-3 library, keep the failed root for inspection, and do not publish a manifest · corrupt `run.json` wording is the remaining product gap · Criterion and CodSpeed are additional measurements, not Python tooling substitutes |
