...
| test_prompt_cli.py | 150 | crates/skit-cli/tests/port_test_prompt_cli.rs | in fix (70/80) · **stdin store-error 'taken' FIXED 592d236** · **edit drift hint FIXED 1cac4a4** · runner trim/pick persistence fixed `2c1f6ef` · Unicode `--set` fixed `07dfbce` · add-time unknown-runner voice fixed `3d43a40` · typed value voice fixed `33d9a4a` · compound `.prompt.md` naming fixed `19cc2ff` · missing prompt body exit fixed `0f1fdd9` · argv NUL/limit voices fixed `41f2ec0` · stable-name recovery command quoting fixed `076d04c` · remaining divergences include stdin defaults and runner recovery messages |
