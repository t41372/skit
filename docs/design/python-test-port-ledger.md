...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs + crates/skit-store/tests/mutations.rs | **Phase 4 re-audit required (65/85 in the CLI owning target)** · installer failures now capture typed exit/stderr, retain the actionable trusted cause, filter fixed npm noise, and preserve every authoritative byte on rollback `b1ad9ac` · removal wait/refusal owners moved to the real store lock boundary `a219125` with persistent-inode and byte-exact no-write assertions · a static audit still finds 35 ignored ABSENT attributes; several reasons are stale after stderr/staging/cleanup fixes, but each must be rehomed, implemented, or honestly closed before parity can be claimed |
...
| test_source_default_semantics.py | 19 | crates/skit-form/tests/port_test_source_default_semantics.rs + skit-language semantic units | done (9: 7 form + 2 rehomed semantic guards) · 10 cross-crate (assemble→skit-application, edit_specs→skit-cli) |
...
| test_editor.py | 50 | crates/skit-cli/tests/port_test_editor.rs + private CLI editor-resolution units | **Phase 4 helper-seam re-audit required (47/4 across 50 frozen owners + 1 Rust regression)** · nine resolution owners now exercise one production precedence/tokenization helper, including the Windows quoting and backslash dialect `cf8ebba` · four ignored open-editor/open-entry helper rows still carry ABSENT/MUST-FIX reasons and must be compared with the stronger public PTY owners before parity is final · add-editor requires both terminal streams, asks/validates a missing name before authoring, and uses true PTY siblings `7fd57de` · name conflict fixed `592d236` · editor resolution/Saved/reference guard fixed `1cac4a4` |
...
| test_edit.py | 14 | crates/skit-cli/tests/port_test_edit.rs + skit-domain source-management units | **Phase 4 API-owner re-audit required (7/7 frozen owners plus Rust support)** · candidate management now has one public typed domain result; already-managed/not-candidate requests warn in order while valid siblings commit once, JSON stays pure, secrets purge, and invalid-only batches write nothing `85ecf69` · seven ignored direct reconcile-helper rows still carry ABSENT/MUST-FIX reasons; their behavior may now be split across typed domain/CLI owners, but that mapping must be proved before closure · malformed `--prompt` warns without touching source/meta/state `ccec2c6` · command-source editor contract activated `5d0d888` · reference source-management refusal exit fixed `4746c36` |
...
| test_path_tui.py | 61 | crates/skit-tui/tests/port_test_path_tui.rs | **release blocker (32/29)** · current-directory picker, PageUp/PageDown, and picked-path escaping are green (`9718a1a`, `98b6bc8`, `0fd6d85`) · a Phase 4 three-way audit proved that 25 ignores are genuine shipped ghost-completion gaps, not private/cross-crate/architecture closures; async capped scan, activation, rendering, Right acceptance, error/no-cache behavior, secret exclusion, and vanished-origin composition remain to implement |
