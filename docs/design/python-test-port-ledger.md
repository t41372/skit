...
| test_js_deps.py | 143 | crates/skit-cli/tests/port_test_js_deps.rs | done (54) · 115 cross-crate/absent · 10 divergences (manifest name-key, staging cwd, stamp path) · input-order/empty-requirement fixed `54bf6c6` · mirror empty-value fixed `c3e02b8` · reference implicit scan fixed `6aedb9b` · installer argv/fallback fixed `05e8b9d` · reference refusal voices fixed `dc770fc` · transactional artifact cleanup and aged injected sweep fixed `cc0ead1` · 6 verifier-caught defects fixed |
