...
**Port COMPLETE. Impl-fix pass WELL UNDERWAY: 59 fix commits landed, 144 FAILING CONTRACTs closed
(142 removed/un-ignored + 2 re-labeled white-box), 2 stubs promoted, 2 owed white-box units added.
...
142 FAILING CONTRACT attributes remain (§5 has the per-file map). JS deps, run-set, prompt-kind,
prompt UTF-8,
...
grep -rh '#\[ignore = "FAILING CONTRACT' crates --include='*.rs' | wc -l   # => 142
```
...
## 5. REMAINING work — 142 FAILING CONTRACTs by file (fix-pass backlog)

Recommended: keep banking coherent clusters, one commit per cluster. Biggest-first is fine now that
the loop is proven; `edit_declared` (params/edit) last as before. Counts are exact as of `53bdb1c`.

- **27 port_test_prompt_cli.rs + 0 port_test_prompt_kind.rs + 0 port_test_prompt_utf8.rs — the
