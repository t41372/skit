...
def test_doctor_json_uv_missing_on_an_empty_library_exits_0(monkeypatch):
    """--json keeps exit parity with the human output: an empty library without uv is
    healthy (nothing needs uv yet), so 0 — and the payload still carries the null uv."""
