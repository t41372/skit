...
from conftest import patch_run_entry, plan_cache_key, without_block
from skit import argstate, argv_text, config, flows, store, tui, tui_form
...
async def test_an_unverifiable_record_stops_the_launch_lane(tmp_path, quiet_run, monkeypatch):
    """A claim that cannot VERIFY (a meta corrupting mid-keypress) fails closed since
    round 18: acting on the held snapshot could operate on whoever owns the slug now,
    so the lane stops with the error on the status line — no keypress handler crashes,
    and nothing launches. (Only the pure-display _fresh may degrade to a snapshot.)"""
    ...
    async with app.run_test(size=(100, 40)) as pilot:
        ...
        assert "meta corrupted mid-keypress" in status

    assert "extra" not in quiet_run  # nothing launched
